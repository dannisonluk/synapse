...
export const NODE_CATALOG: Record<AlteryxNodeType, NodeSpec> = {
	...
	TEXT_TO_COLUMNS: {
		...
		category: "Preparation",
		description: "用分隔符或正規表示式把一個字串欄位拆成多個欄位。",
		...
		fields: [
			{ name: "field", kind: "field", label: "來源欄位", required: true },
			{
				...
				kind: "enum",
				label: "切分方式",
				values: ["SEPARATOR", "REGEX"],
				default: "SEPARATOR",
				hint: "SEPARATOR → 字面分隔符；REGEX → 下面的字串當成樣式（可變長度分隔，例如 \\s+）",
			},
			{
				name: "separator",
				kind: "text",
				label: "分隔符 / 樣式",
				default: ",",
				hint: "SEPARATOR 模式是字面比對（不需要跳脫）；REGEX 模式是 RE2 / Rust regex 語法",
			},
			{
				...
				label: "忽略大小寫（REGEX）",
				default: false,
				hint: "只在 REGEX 模式有意義；以 inline (?i) 摺進樣式",
			},
			{
				name: "outputColumns",
				kind: "textList",
